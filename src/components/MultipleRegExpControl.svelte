<script lang="ts">
	...
	export let apply: (args: string[]) => Promise<void> = (_: string[]) =>
		Promise.resolve();
	function revert() {
		patterns = [...originals];
	}
	...
	$: modified = patterns.map((e, i) =>
		e != (originals?.[i] ?? "") ? MARK_MODIFIED : "",
	);
...
<ul>
	{#each patterns as pattern, idx}
		...
		<li>
			<label>{modified[idx]}{statusName[idx]}</label><input
				type="text"
				bind:value={pattern}
				class={modified[idx]}
			/>
	...
	<li class="buttons">
		<button
			...
			disabled={statusName.some((e) => e === CHECK_NG) ||
				modified.every((e) => e === "")}
			>Apply
		</button>
		<button
